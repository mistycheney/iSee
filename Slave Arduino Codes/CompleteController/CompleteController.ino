...
unsigned long startTime=0, lastTime=0, comTimeNow=0, comTimeLast=0;
...
void loop(){
  ...
  if (EndPath==1){
    ...
    CircObs=1;
    SegmentDone=0;}
  if (CircObs!=1/*maybe a NewSegment==1*/){
    waypointX[0]=state[0];
    waypointY[0]=state[1];
  ...
  for (int c=1; c<=10; c=c+1){
    ...
    Serial.println(step0[c]);
    Serial.println(step1[c]);
    Serial.println(step2[c]);
    Serial.println(step3[c]);
    Serial.println(step4[c]);
  }
  Drive();
  }
...
void Drive(){
  ...
  Speed();
  comTimeNow=millis();
  if (comTimeNow-comTimeLast>100){
  sendWire(state[0]);
  sendWire(state[1]);
  sendWire(state[2]);
  }
  if(abs(state[0]-reference[0])>0.3 && abs(state[1]-reference[1])>0.3 && Recovery==0){
  ...
  if((abs(state[0]-reference[0])<0.05 && abs(state[1]-reference[1])<0.05 && CircObs==0) || (abs(state[2]-reference[2])<0.03 && CircObs==1)){
    Recovery=0;
    if(goalnum==10){
      analogWrite(PWMA, 0); 
      analogWrite(PWMB, 0); 
...
void sendWire (float value){
Wire.beginTransmission(I2C_MASTER_ADDRESS); // transmit to device #44 (0x2c)
					    // device address is specified in datasheet
byte b[4];
memcpy(b, &value, sizeof(float));
                  // note that Wire.write can only send one byte at each call
Wire.write(b[3]); // note that AVR internal storage is little-endian, meaning MSB is stored in high address;
Wire.write(b[2]); // should send in MSB-first order
Wire.write(b[1]);
Wire.write(b[0]);
