 ...
 *      Author: Edoardo cacciavillani & Yuncong Chen
 */
...
int torquePWM[4];
boolean direction[4];
uint8_t NumOfSetpoints = 4;

unsigned long startTime = 0, lastTime = 0 /*, setpointTime = 0 , controlTime = 0*/;
...
const float K[4][6]={{ 0.0,   1.0825,    0.6250,     0.0,    1.0479,    0.1712},
        			 {-1.0825,   0.0,    0.6250,    -1.0479,    0.0,    0.1712},
        			 {0.0,   -1.0825,    0.6250,     0.0,   -1.0479,    0.1712},
        			 {1.0825,    0.0,    0.6250,     1.0479,    0.0,    0.1712}};

const float qTOv[3][4]={{     0,   -0.5000,         0,    0.5000},
        				{0.5000,   -0.0000,   -0.5000,    0.0000},
        				{1.5385,    1.5385,    1.5385,    1.5385}};
...
void Drive() {
	...
	while (!isFINSEG()) {
		...
		compute_PWM_DIR();
		print_array("uu",uu,6,4);
		print_array("u", u, 4, 4);
		print_array("Input", input, 4, 4);
		//print_array("torquePWM", torquePWM, 4, 3);
...
//		I2Csend();
		...
		// too far from reference or too much time from a waypoint to another one (robot stopped)

		//controlTime=millis();

		if (abs(state[0]-setpoint0[curr_sp]) > 0.3
				|| abs(state[1]-setpoint1[curr_sp]) > 0.3 && Recovery == 0
					/*|| (controlTime-setpointTime > 5000 && Recovery == 0)*/) {
...
void compute_PWM_DIR() {
	...
	for (uint8_t i = 0; i <= 3; i++) {
		...
		torquePWM[i] = (input[i] > lowerlimit) ? (input[i] / range) * 40 + 38 : 0;
	}
}
...
void BezierInterp() {
	...
	math.MatrixMult((float*) BCoeff, (float*) waypointY, 11, 4, 1,
			(float*) setpoint1);
	float dx1, dy1, dx2, dy2, dxy1, dxy2, dir, angle1, angle2, dangle, absangle;
	dx1 = cos(state[2]);
	dy1 = sin(state[2]);
	angle2 = atan2(setpoint1[1] - setpoint1[0],setpoint0[1] - setpoint0[0]);
	Serial.print("angle2 - atan2");
	Serial.print(" ");
	Serial.print(angle2,6);
	dx2 = cos(angle2);
	dy2 = sin(angle2);
    dir = dx1 * dy2 - dx2 * dy1;
    absangle=abs(angle2-state[2]);
	if(absangle>0.005){
          if(absangle>PI){dangle=2*PI-absangle;}
          else{dangle=absangle;}
          if (dir >= 0)
            {seqTH[0] = dangle;}
          else
            {seqTH[0] = -dangle;}
	...
	for (uint8_t i = 1; i <= 9; i = i + 1) {
      angle2 = atan2(setpoint1[i + 1] - setpoint1[i],setpoint0[i + 1] - setpoint0[i]);
	  dx2 = cos(angle2);
      dy2 = sin(angle2);
      //Serial.print("angle2 - atan2");
      //Serial.print(" ");
      //Serial.print(angle2,6);
	  angle1=atan2(setpoint1[i] - setpoint1[i - 1],setpoint0[i] - setpoint0[i - 1]);
	  dx1 = cos(angle1);
	  dy1 = sin(angle1);
//	  Serial.print("angle1 - atan2");
//	  Serial.print(" ");
//	  Serial.print(angle1,6);
  	  dir = dx1 * dy2 - dx2 * dy1;
      absangle=abs(angle2-angle1);
      if(absangle>0.005){
		if(absangle>PI)
			{dangle=2*PI-absangle;}
		else
			{dangle=absangle;}
		if (dir >= 0)
			{seqTH[i] = dangle;}
...
//		Serial.print("Vref*cos(orient[i])");
//		Serial.print("  ");
//		Serial.print(setpoint3[i],6);
		setpoint4[i] = Vref * sin(orient[i - 1]);
//		Serial.print("Vref*sin(orient[i])");
//		Serial.print("  ");
//		Serial.print(setpoint4[i],6);
	}
	setpoint2[10] = orient[9];
	setpoint3[10] = 0.0;
	setpoint4[10] = 0.0;
	setpoint5[10] = 0.0;
	for (uint8_t i = 1; i <= 10; i = i + 1){
		Serial.print("setpoint ");
			Serial.print(i);
			Serial.print(": ");
			Serial.print(setpoint0[i]);
			Serial.print(" ");
			Serial.print(setpoint1[i]);
			Serial.print(" ");
			Serial.print(setpoint2[i]);
			Serial.print(" ");
			Serial.print(setpoint3[i]);
			Serial.print(" ");
			Serial.print(setpoint4[i]);
			Serial.print(" ");
			Serial.println(setpoint5[i]);
	}
}
...
void CircularObservation() {
	...
	for (uint8_t i = 1; i <= 4; i++) {
		...
		setpoint3[i] = 0;
		setpoint4[i] = 0;
		setpoint5[i] = 0.2; //we help the rotation setting constant angular speed
	}
	setpoint5[10] = 0;
	//we may even change the coefficient on the k matrix for penalty on angular speed.
}
...
void RecoveryMode() {
	...
	setpoint3[curr_sp] = 0.3 * cos(setpoint2[curr_sp]);
	setpoint4[curr_sp] = 0.3 * sin(setpoint3[curr_sp]);
	Recovery = 1;
}
